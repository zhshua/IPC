...
def scores(mydb, path, fx_startid, time_startid, time_len):
    ...
    ports_score = ((min_ports - ports) / (min_ports + ports)) * 100
    login_score = ((min_login - login) / (min_login + login) + (min_fail - fail) / (min_fail + fail)) / 2 * 100
    time_score = ((0 - time) / time) * 100
    white_score = ((80 / 9) * (whi * whi) - (62 / 9) * whi - 1) * 100
    total_score = (dos_score + ports_score + login_score + time_score + white_score) / 5
    ...
    # print(sql)
    val = (str(int(total_score)), str(result), str(fx_startid))
