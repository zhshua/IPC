...
def svm(path1, mydb, jx_startid):
    ...
    data1 = np.loadtxt("C:\\Users\\ZS\\Desktop\\解析的\\svmdata.txt", dtype=int, delimiter=',')  # 读取样本集，这里样本集不做模型训练，用于data2的同步归一化

    sqlcom = 'select * from 分析表 LIMIT ' + str(jx_startid - 1) + ',1; '
    df = pd.read_sql(sqlcom, con=mydb)
    if (is_Chinese(df.iloc[0, 8]) == True):
        result='不存在攻击'
    else:
        ...
        clf = joblib.load("C:\\Users\\ZS\\Desktop\\解析的\\train_model.m")  # 读取模型
